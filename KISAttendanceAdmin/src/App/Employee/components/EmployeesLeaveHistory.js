...
function LeaveHistory() {
  ...
  const getUserLeaves = async (initValue, searchInitValue, optionInitValue, page) => {
    try {
      setPage(page + 1);
      setLoading(true);
      await httpClient
        .get(`${LEAVES.GET_ALL_EMPLOYEE_LEAVES}?page=${page + 1}&alphaTerm=${initValue}&searchText=${searchInitValue}&optionTerm=${optionInitValue}`)
        .then((res) => {
          if (res.status === 200) {
            if (!updatedList) {
  ...
  return (
    <>
      <div className="main_content_panel container employees-leaves-page">
                ...
                <div className="d-lg-flex w-100 justify-content-end">
                  <div className="col-lg-3 me-lg-3 mb-3 mb-lg-0">
                    <div className="dropdown_icon">
                      <select className="form-control rounded-50"
                        aria-label="Default select example" onChange={handleOptionValue} value={optionValue}>
                  ...
                  <tbody>
                    {updatedList?.length > 0 &&
                      updatedList.map((leave, i) => (
                        <tr key={i}>
                          <td className="text-nowrap">{leave?.user_id[0]?.name}</td>
                          <td className="text-nowrap">
                            {leave?.user_id[0]?.emp_id}
                          </td>
                          <td className="text-nowrap">
                            <span> {moment(leave.from).format("L")}</span>
                            {(leave.type === "Half Day" || leave.type === "Short Leave" || moment(moment(leave.from).format("YYYY/MM/DD")).isSame(moment(moment(leave.to).format("YYYY/MM/DD")))) ? "" : '-' + moment(leave.to).format("L")}
                          ...
                          <td>
                            {leave.status === "pending" && (
                              <div className="btn-group dropend">
                                ...
                                <ul className="dropdown-menu" aria-labelledby="dropdownMenu2">
                                  <li><button className="dropdown-item text-success" type="button" onClick={(e) => approveLeave(leave._id)}>Approve</button></li>
                                  <li><button className="dropdown-item text-danger" type="button" onClick={(e) => setShow({ open: true, leaveId: leave._id })}>Reject</button></li>
                            ...
                            {leave.status === "approved" && (
                              <div className="btn-group dropend">
                                ...
                                <ul className="dropdown-menu" aria-labelledby="dropdownMenu2">
                                  <li><button className="dropdown-item text-dark" type="button" onClick={(e) => canceLeave(leave._id)} >Cancel</button></li>
                                </ul>
                              </div>
                            )}
                          </td>
                ...
                {updatedList.length <= 0 || updatedList?.length < total && (
                  <div className="text-center">
                    ...
                    <InfiniteScroll
                      ...
                      next={fetchMoreData}
                      hasMore={true}
                      loader={initialValue || searchValue || optionValue ? "" : <h4>Loadings...</h4>}
                    >
