...
function Thought() {
  ...
  const [showPresentEmployees, setShowPresentEmployees] = useState(true);
  ...
  const [isDisplayValue, setIsDisplayValue] = useState("");
  ...
  const confirmDeleteThought = async () => {
    ...
    try {
      await httpClient
        ...
        .then((res) => {
          if (res.status === 200) {
            ToastsStore.success("Thought Deleted Successfully");
            getThoughts();
            setshowDelThought(false);
          }
        })
        ...
        if (showIsDisplay === true) {
          ...
          if (values.thought && editorErrorValue === "") {
            await httpClient
              ...
              .then((res) => {
                ToastsStore.success("Thought Added Successfully");
        ...
        if (showIsDisplay === true) {
          ...
          if (values.thought && editorErrorValue === "") {
            await httpClient
              ...
              .then((res) => {
                ToastsStore.success("Thought Updated Successfully");
                window.location.reload();
                getThoughts();
                executeMainScroll();
              })
